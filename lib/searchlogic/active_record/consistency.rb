  ...
  module ActiveRecord
    ...
    module Consistency
      def self.included(klass)
        # klass.class_eval do
        #   alias_method_chain :build_joins, :singularity
        #   alias_method_chain :build_joins, :consistent_conditions
        #   alias_method_chain :build_joins, :merged_duplicates
        # end
      end
      ...
      def build_joins_with_singularity(*args)
        joins = build_joins_without_singularity(*args)
      ...
      # combining scopes.
      def build_joins_with_consistent_conditions(*args)
        joins = build_joins_without_consistent_conditions(*args)
        joins.collect do |j|
      ...
      def build_joins_with_merged_duplicates(*args)
        args << "" if !Thread.current["searchlogic_delegation"]
        joins = build_joins_without_merged_duplicates(*args)
      end
    end
  end
end